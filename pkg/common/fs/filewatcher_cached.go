...
type CachedFileWatcher interface {
	...
	Add(path string, cb CachedWatchCallback) error
...
func (ccw *cachedFileWatcher) callback(path string, cb CachedWatchCallback) WatchCallback {
	return func() {
		ccw.m.Lock()
		defer ccw.m.Unlock()
		...
		cb(ccw.watchedFiles[path])
	}
}

type CachedWatchCallback func(content []byte)

// Add a file path to be watched.
func (ccw *cachedFileWatcher) Add(path string, cb CachedWatchCallback) error {
	if err := ccw.cw.Add(path, ccw.callback(path, cb)); err != nil {
		return err
	}
